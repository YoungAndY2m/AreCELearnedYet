...
class MADE(nn.Module):
    ...
    def __init__(
        ...
        Args:
          ...
          embed_threshold: embed the column if domain size is larger than this threshold
        """
        super().__init__()
        L.info(f'fixed_ordering: {fixed_ordering}, seed: {seed}, natural_ordering: {natural_ordering}')
        self.nin = nin
        assert input_encoding in [None, 'one_hot', 'binary', 'embed', 'auto']
        self.input_encoding = input_encoding
        assert output_encoding in ['one_hot', 'embed', 'auto']
        ...
        if fixed_ordering is not None:
            assert num_masks == 1
            L.info(f'** Fixed ordering {fixed_ordering} supplied, ignoring natural_ordering')
        ...
        L.info(f'encoded_bins (output): {encoded_bins}')
        L.info(f'encoded_bins (input): {self.input_bins_encoded}')
        ...
        if self.output_encoding == 'auto':
            assert self.input_encoding == 'auto'
        ...
        if self.input_encoding == 'embed':
            ...
            self.embeddings = nn.ModuleList()
            for i, dist_size in enumerate(self.input_bins):
                if dist_size > self.embed_threshold:
                    embed = nn.Embedding(dist_size, self.embed_size)
                else:
                    embed = None
                self.embeddings.append(embed)
    ...
    def _get_input_encoded_dist_size(self, dist_size):
        if self.input_encoding == 'embed':
            ...
            # use embedding for columns less than threshold, o.w. binary
            if dist_size > self.embed_threshold:
                dist_size = self.embed_size
            else:
                dist_size = max(1, int(np.ceil(np.log2(dist_size))))
    ...
    def _get_output_encoded_dist_size(self, dist_size):
        if self.output_encoding == 'embed':
            ...
            dist_size = max(1, int(np.ceil(np.log2(dist_size))))
        elif self.output_encoding == 'auto':
            # use embedding for columns less than threshold, o.w. one hot
            if dist_size > self.embed_threshold:
                dist_size = self.embed_size
        return dist_size
        ...
        if invoke_order is not None:
            ...
            for i in range(len(self.orderings)):
                if np.array_equal(self.orderings[i], invoke_order):
                    found = True
                    break
            assert found, 'specified={}, avail={}'.format(invoke_order, self.orderings)
    ...
    def name(self):
        ...
        if self.residual_connections:
            n = 'resmade'
        n += '_hid' + ','.join(str(h) for h in self.hidden_sizes)
        if self.input_encoding in ['embed', 'auto']:
            n += '_emb' + str(self.embed_size)
        if self.epoch is not None:
            n += '_ep' + str(self.epoch)
        if self.num_masks > 1:
            n += '_{}masks'.format(self.num_masks)
        if not self.natural_ordering:
            n += '_nonNatural'
        if not self.do_direct_io_connections:
            n += '_nodirectIo'
        #  n += ('-no' if not self.do_direct_io_connections else '-') + 'directIo'
        n += '_{}In{}Out'.format(self.input_encoding, self.output_encoding)
        if self.input_encoding == 'auto':
            n += '_embThres' + str(self.embed_threshold)
        if not self.input_no_emb_if_leq:
            n += '_noInNoEmbIfLeq'
        if not self.column_masking:
            n += '_noColmask'
        if self.fixed_ordering is not None:
            n += '_order' + ','.join(map(str, self.fixed_ordering))
        return n
    ...
    def AutoEncode(self, data, natural_col=None, out=None):
        ...
        if natural_col is not None:
            # inference path
            dom_size = self.input_bins[natural_col]
            if dom_size > self.embed_threshold:
            ...
            y = [None] * self.nin
            # train path
            for i, dom_size in enumerate(self.input_bins):
                if dom_size > self.embed_threshold:
                    # copy from Embed
                    col_i_embs = self.embeddings[i](data[:, i])
                    if not self.column_masking:
                        ...
                        batch_mask = torch.clamp(
                            torch.dropout(torch.ones(bs, 1, device=data.device),
                                          p=dropout_p(),
                                          train=self.training), 0, 1)
                        y[i] = batch_mask * col_i_embs + (1. - batch_mask) * dropped_repr

                else:
                    # copy from ToBinaryAsOneHot
                    data_np = data.narrow(1, i, 1)
                    binaries = (data_np & self.bin_as_onehot_shifts[i]) > 0
                    y[i] = binaries
                    ...
                    if self.column_masking:
                        ...
                        binaries = binaries.to(torch.float32,
                                               non_blocking=True,
                                               copy=False)
                        y[i] = batch_mask * binaries + (1. - batch_mask) * dropped_repr
    ...
    def logits_for_col(self, idx, logits):
        ...
        if idx == 0:
            ...
            logits_for_var = logits[:, self.logit_indices[idx - 1]:self.
                                    logit_indices[idx]]
        if self.output_encoding == 'one_hot':
