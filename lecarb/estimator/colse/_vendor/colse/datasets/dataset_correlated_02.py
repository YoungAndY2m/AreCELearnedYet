...
dataset_dir = get_data_path() / "correlated_02/"
IS_DEQUANTIZE = False

def generate_dataset_correlated_02(**kwargs):
    nrows = kwargs.get("no_of_rows", 500_000)
    selected_cols = kwargs.get("selected_cols", None)

    """ Load dataset"""
    dataset_path = get_data_path() / "correlated_02/original.csv"
...
def get_queries_correlated_02(**kwargs) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
