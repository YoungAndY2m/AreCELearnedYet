...
def generate_dataset(**kwargs):
    dataset_type = DatasetNames.CENSUS_DATA
    ...
    attr1 = df["age"].to_numpy()[:nrows]
    attr2 = df["workclass"].to_numpy()[:nrows]
    attr3 = df["education"].to_numpy()[:nrows]
    attr4 = df["education_num"].to_numpy()[:nrows]
    attr5 = df["marital_status"].to_numpy()[:nrows]
    attr6 = df["occupation"].to_numpy()[:nrows]
    attr7 = df["relationship"].to_numpy()[:nrows]
    attr8 = df["race"].to_numpy()[:nrows]
    attr9 = df["sex"].to_numpy()[:nrows]
    attr10 = df["capital_gain"].to_numpy()[:nrows]
    attr11 = df["capital_loss"].to_numpy()[:nrows]
    attr12 = df["hours_per_week"].to_numpy()[:nrows]
    attr13 = df["native_country"].to_numpy()[:nrows]
    ...
    new_df = pd.DataFrame(data, columns=[f"{ROW_PREFIX}{i}" for i in range(1, 14)])
...
def get_queries_census(**kwargs) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
